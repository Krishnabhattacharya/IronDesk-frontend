...
class Times {
  ...
  static String timestamp2String(
    int timestamp, {
  ...
  static String converAmPmToMilitaryTiming(String initialTiming) {
    ...
    if (int.parse(initialTiming.split(":")[0]) == 12) {
      ...
      result = int.parse(
        initialTiming.split(":")[0] + initialTiming.split(":")[1].substring(0, 2),
      );
    }
  ...
  static int getHRSFromMilitary(String timing) {
    ...
    if (initialTiming.length < 4) {
      initialTiming = List.generate(4 - initialTiming.length, (index) => "0").toList().join() + initialTiming;
    }
    return int.parse(
  ...
  String getDatefromDateTime(DateTime dateTime) {
    final DateTime parsedDateTime = DateTime.parse(dateTime.toString());
